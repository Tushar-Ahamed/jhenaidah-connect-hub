...
export function DistrictMap() {
  const [active, setActive] = useState(UPAZILA_LIST[0].slug);
  const current = UPAZILA_LIST.find((u) => u.slug === active) ?? UPAZILA_LIST[0];
    ...
    <div className="grid gap-8 lg:grid-cols-[1.2fr_1fr]">
      <div className="relative aspect-[4/5] overflow-hidden rounded-3xl border border-border bg-card shadow-xl">
        <svg viewBox="0 0 100 100" className="absolute inset-0 h-full w-full">
          ...
          <path
            ...
            fill="url(#districtFill)" stroke="var(--brand-green-dark)" strokeWidth="0.6" strokeDasharray="1.4 1.2"
          />
          {UPAZILA_LIST.map((u) => {
            const isActive = u.slug === active;
            return (
              <g key={u.slug} onMouseEnter={() => setActive(u.slug)} onFocus={() => setActive(u.slug)} className="cursor-pointer">
                <circle cx={u.map.x} cy={u.map.y} r={isActive ? 3.4 : 2.4}
                  fill={isActive ? "var(--brand-red)" : "var(--brand-green)"}
                  stroke="white" strokeWidth="0.6" className="transition-all duration-300" />
                {isActive && (
                  <circle cx={u.map.x} cy={u.map.y} r="5.6" fill="none" stroke="var(--brand-red)" strokeWidth="0.5" opacity="0.6">
                ...
                <text x={u.map.x} y={u.map.y - 4.5} textAnchor="middle" className="pointer-events-none"
                  style={{ fontSize: "2.6px", fontWeight: 700, fill: isActive ? "var(--brand-red)" : "var(--foreground)" }}>
                  {u.name}
                </text>
              </g>
            );
      ...
      <div className="flex flex-col">
        <div className="card-elevated p-6">
          <div className="text-xs font-semibold uppercase tracking-wider text-brand-red">নির্বাচিত উপজেলা</div>
          <h3 className="mt-2 text-2xl font-bold">{current.name}</h3>
          <p className="mt-2 text-sm text-muted-foreground">{info?.intro ?? "উপজেলার পরিচিতি শীঘ্রই আপডেট করা হবে।"}</p>
          <div className="mt-5 grid grid-cols-3 gap-2 text-center">
            <div className="rounded-lg bg-secondary p-3">
              <div className="text-lg font-bold text-primary">{counts[current.name] ?? 0}</div>
              <div className="text-[11px] text-muted-foreground">সদস্য</div>
            </div>
            <div className="rounded-lg bg-secondary p-3">
              <div className="truncate text-sm font-bold">{s.president ?? "—"}</div>
              <div className="text-[11px] text-muted-foreground">সভাপতি</div>
            </div>
            <div className="rounded-lg bg-secondary p-3">
              <div className="truncate text-sm font-bold">{s.secretary ?? "—"}</div>
              <div className="text-[11px] text-muted-foreground">সম্পাদক</div>
            </div>
          </div>
          <Link to="/upazila/$slug" params={{ slug: current.slug }}
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          {UPAZILA_LIST.map((u) => (
            <button key={u.slug} onClick={() => setActive(u.slug)}
              className={`rounded-lg border px-3 py-2 text-left text-sm transition ${
                u.slug === active ? "border-brand-red bg-accent font-semibold text-accent-foreground"
