...
function AdminNotices() {
  ...
  async function save(row: any) {
    const payload = {
      ...
      level: row.level,
      upazila: row.level === "upazila" ? row.upazila : null,
    };
    const { error } = row.id
  ...
  return (
    <PageShell title="নোটিশ পরিচালনা">
      ...
      {editing && (
        <Editor row={editing} onSave={save} onCancel={() => setEditing(null)} isDistrict={!!isDistrict} upazilaScope={upazilaScope} />
      )}

      <div className="mt-4 space-y-3">
        {notices?.map((n: any) => (
          <div key={n.id} className="card-elevated flex items-start justify-between gap-3 p-4">
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{n.level === "district" ? "জেলা" : n.upazila} • {n.event_date || new Date(n.created_at).toLocaleDateString("bn-BD")}</div>
              <div className="font-bold">{n.title}</div>
              <p className="text-sm text-muted-foreground">{n.body}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => setEditing(n)} className="rounded p-2 hover:bg-accent"><Pencil className="h-4 w-4" /></button>
              <button onClick={() => del(n.id)} className="rounded p-2 text-brand-red hover:bg-accent"><Trash2 className="h-4 w-4" /></button>
            </div>
          </div>
        ))}
      </div>
...
function Editor({ row, onSave, onCancel, isDistrict, upazilaScope }: any) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onSave(f); }} className="card-elevated space-y-3 p-6">
      ...
      <textarea placeholder="বিস্তারিত" value={f.body || ""} onChange={(e) => setF({ ...f, body: e.target.value })} rows={4} className="w-full rounded-lg border border-input bg-background p-3 text-sm" />
